...
// ?
     function hasClass(markup, cssClass){
        var regex = new RegExp("class\\s?=\\s?[\'\"](:?\\w+\\s)?" + cssClass +"(:?\\s\\w+)?[\"\']", "gmi");
        console.log(regex.test(markup) +  " para a classe "  + cssClass);
        return regex.test(markup);
    }
  
  Existe a container, text, date, excerpt. A Main não existe
